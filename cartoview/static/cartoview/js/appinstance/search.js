...
(function () {
				...
				module.load_categories = function ($http, $rootScope, $location) {
								...
								$http
												.get(CATEGORIES_ENDPOINT, {params: params})
												.success(function (data) {
																if ($location.search().hasOwnProperty('category__identifier__in')) {
																				data.objects = module.set_initial_filters_from_query(data.objects, $location.search()['category__identifier__in'], 'identifier');
																}
				...
				module.load_keywords = function ($http, $rootScope, $location) {
								...
								$http
												.get(KEYWORDS_ENDPOINT, {params: params})
												.success(function (data) {
																if ($location.search().hasOwnProperty('keywords__slug__in')) {
																				data.objects = module.set_initial_filters_from_query(data.objects, $location.search()['keywords__slug__in'], 'slug');
																}
				...
				module.load_regions = function ($http, $rootScope, $location) {
								...
								$http
												.get(REGIONS_ENDPOINT, {params: params})
												.success(function (data) {
																if ($location.search().hasOwnProperty('regions__name__in')) {
																				data.objects = module.set_initial_filters_from_query(data.objects, $location.search()['regions__name__in'], 'name');
																}
				...
				module.load_owners = function ($http, $rootScope, $location) {
								...
								$http
												.get(OWNERS_ENDPOINT, {params: params})
												.success(function (data) {
																if ($location.search().hasOwnProperty('owner__username__in')) {
																				data.objects = module.set_initial_filters_from_query(data.objects, $location.search()['owner__username__in'], 'identifier');
																}
	 ...
	 */
		 ...
		 */
								...
								$scope.$watch('total_counts', function () {
												...
												// In case the user is viewing a page > 1 and a subsequent query returns less
												// pages, then reset the page to one and search again.
		 ...
		 */
								$scope.multiple_choice_listener = function ($event) {
												...
												if ($scope.query.hasOwnProperty(data_filter)) {
																...
																// When in the location are passed two filters of the same type then they are
																// put in an array otherwise is a single string
												...
												if (element.hasClass('active')) {
																...
																query_entry.splice(query_entry.indexOf(value), 1 // if is not active then activate it
																);
