...
(function() {
	...
	module.run(function($http, $rootScope, $location) {
		...
		if ($location.search().hasOwnProperty('type__in')) {
			...
			if (types instanceof Array) {
				for (var i = 0; i < types.length; i++) {
					$('body').find("[data-filter='type__in'][data-value=" + types[i] + "]")
						.addClass('active');
				}
			} else {
				$('body').find("[data-filter='type__in'][data-value=" + types + "]").addClass(
					'active');
			}
		}
		...
		if ($location.search().hasOwnProperty('order_by')) {
			...
			$('body').find("[data-filter='order_by'][data-value=" + sort + "]").addClass(
				'selected');
		}
